...
def get_area_codes_called_from_bangalore(calls_summary):
    ...
    Algorithmic complexity = O(n log n) for worst case

    There is O(n log n) + O(n) to use the built-in sorted method and
    ...
    called from Bangalore in this case. However, this reduces to O(n log n) in the
    context of approximating runtime complexity.
    """
